...
float solveSlope(struct line equation){
    /* use given formula */
    float y = equation.point1.y - (equation.point2.y);
    float x = equation.point1.x - (equation.point2.x);
    float slope = y/x;  
    return slope;
}
...
float* solveMidpoint(struct line equation){
    /* base on the given formula */
    float x = (equation.point1.x + equation.point2.x)/2;
    float y = (equation.point1.y + equation.point2.y)/2;
...
void slopeInterceptForm(struct line equation){
    /* to solve for b */
    /* b = y - mx */
    float b = equation.point1.y - (equation.slope*equation.point1.x);
...
int main(void){
    ...
    scanf ("%f %f", &equation.point1.x, &equation.point1.y);    

    /* store values in the structure members so we can access it later on*/
    printf("Enter coordinates of point 2: ");
    scanf ("%f %f", &equation.point2.x, &equation.point2.y);
